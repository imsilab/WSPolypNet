...
from model import SlowR50Binary, count_trainable_parameters
...
def build_optimizer(
    ...
    # gradients during epochs 1-5 and begin updating immediately when unfrozen.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train one-logit Kinetics-pretrained Slow R50 and evaluate CAM CorLoc."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Freeze the backbone for this many initial epochs (default: 5).",
    )
...
def main() -> int:
    ...
    model = SlowR50Binary(pretrained=args.resume is None).to(device)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if stage != active_stage:
            ...
            backbone_is_in_optimizer = any(
                ...
                if "backbone.blocks.5.proj" not in name
            )
            print(
