...
from model import SlowR50Binary
from train import (
...
def first_backbone_parameter(model: SlowR50Binary) -> torch.nn.Parameter:
    return next(
        parameter
...
def main() -> int:
    ...
    model = SlowR50Binary(pretrained=True).to(device)
    ...
    print("Slow R50 whole-video forward: OK")
