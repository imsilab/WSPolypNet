from __future__ import annotations

import torch
import torch.nn.functional as F
from pytorchvideo.models.hub import x3d_m
from torch import nn


class X3DMBinary(nn.Module):
    """Kinetics-pretrained X3D-M with one binary-classification logit."""

    def __init__(self, pretrained: bool = True) -> None:
        super().__init__()
        self.backbone = x3d_m(pretrained=pretrained, progress=True)
        head = self.backbone.blocks[-1]
        in_features = head.proj.in_features
        # X3D-M's Kinetics head uses a fixed 16x7x7 pool. Keep its pretrained
        # pointwise projection but remove only that pool so every input frame
        # contributes to the feature map and adaptive global pooling below.
        head.pool.pool = nn.Identity()
        head.proj = nn.Linear(in_features, 1)
        head.activation = None

    def forward(
        self, video: torch.Tensor, return_features: bool = False
    ) -> torch.Tensor | tuple[torch.Tensor, torch.Tensor]:
        x = video
        for block in self.backbone.blocks[:-1]:
            x = block(x)
        head = self.backbone.blocks[-1]
        features = head.pool(x)
        classifier_input = head.dropout(features) if head.dropout is not None else features
        pooled = F.adaptive_avg_pool3d(classifier_input, output_size=1).flatten(1)
        logits = self.classifier(pooled)
        if return_features:
            return logits, features
        return logits

    @property
    def classifier(self) -> nn.Linear:
        return self.backbone.blocks[-1].proj

    def set_backbone_trainable(self, trainable: bool) -> None:
        for parameter in self.backbone.parameters():
            parameter.requires_grad = trainable
        for parameter in self.classifier.parameters():
            parameter.requires_grad = True

    def set_training_mode(self, backbone_trainable: bool) -> None:
        self.train()
        if not backbone_trainable:
            # Keep frozen BatchNorm running statistics unchanged as well.
            for block in self.backbone.blocks[:-1]:
                block.eval()
            self.backbone.blocks[-1].eval()
            self.classifier.train()


def count_trainable_parameters(model: nn.Module) -> tuple[int, int]:
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total = sum(p.numel() for p in model.parameters())
    return trainable, total
